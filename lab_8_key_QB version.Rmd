---
title: "ESM 206 Fall 2019 Lab 8 working from key"
author: "Qusai"
date: "11/14/2019"
...
create a subset that only contains obs. for female penguins , then visually explore the distributions for each species
...
#look close to normally distributted, all groupds n > 30  >>> use parametric one-way ANOVA to compare flipper lengths
```



Find some summary statitics using grou_by and summarize:
...
```{r}
leveneTest(flipper_length_mm ~ species, data = penguin_f) # Not significant (we retain the null hypothesis of equal variances) # you will need the car package for the levene's test



# retain the null hypothesis : equal variances
...
Beeswarm plot with mean and standard deciation overlayed
...
ggplot() +
  ...
  geom_point(data = penguin_f_summary, aes(x = species, y = mean_flipper_length), 
             color = "firebrick",
             size = 3) +
  geom_errorbar(data = penguin_f_summary, # gg plot does not make assumptions about where to put the bars, you have to do it manually
                aes(x = species, 
                    ymin = mean_flipper_length - sd_flipper_length,# manually # ymin = mean_flipper_length - sd_flipper_length
                    ymax = mean_flipper_length + sd_flipper_length), # ymax = mean_flipper_length - sd_flipper_length
                color = "firebrick",
                width = 0.1) + # reducing the masssively wide top bar
  theme_light()

```

**Figure 1.** Figure lengths(mm) for Adelie (n= ), Chinstrap (n = ), and Gentoo (n = ), penguins at palmer station , Antarctica. Means indicated by black points; error bars indicate $\pm$ 1 standard deviation. Data Source: 
...
summary(penguin_aov) # you will have to use the summary function to get the way we want to see this.

#aov = ANOVA

# My conclusion is that there is a significant difference  in mean flipper between Adelie, Chinstrap, and Gentoo PEnguins at Palmer Station (F(2,  162) = 411.8, p< oo
...
ONe way ANOCEwith post hoc TUkeys hsd reveals a significant difference in mean female flipper length between all 3 species (gentoo, Adelie, Chinstrap, ) (Stats resuslts)
...
```{r}
party_religion_prop <- party_relig_table %>% 
  janitor::adorn_percentages(denominator = "row") %>% 
  adorn_pct_formatting(digits = 1) %>% 
  adorn_ns(position = "front") #helps with also adding the actual values as well

party_religion_prop


kable(party_religion_prop) %>% 
  kable_styling()
```
